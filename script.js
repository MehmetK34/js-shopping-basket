...
//? urune tikladigim zaman sepete gondermek icin asagidaki kodu yazdim
class UI {
  addToCart(shopping) {
  ...
  removeCart() {
    //? sepete ekledigim urunleri sepetten silmek icin.
    let btnRemove = document.getElementsByClassName("btn-delete");
    let self = this;
    for (let i = 0; i < btnRemove.length; i++) {
      btnRemove[i].addEventListener("click", function () {
        this.parentElement.parentElement.parentElement.remove();
        self.cartCount(); //? sepetten urunleri sildigimde sepetteki urun sayisini sildim.
      });
    }
  }

  cartCount() {
    //? sepetteki urun sayisi icin.
...
for (let i = 0; i < card.length; i++) {
  btnAdd[i].addEventListener("click", function (e) {
    let title = card[i].getElementsByClassName("card-title")[0].textContent; //?ustteki kod ile add to cart a bastigim zaman urunlerin isimlerini aldim.
    let price = card[i].getElementsByClassName("price")[0].textContent;
    let image = card[i].getElementsByClassName("card-img-top")[0].src;
    // console.log(title);
    // console.log(price);
    // console.log(image);
    btnAdd[i].classList.add("disabled"); //? her urune bir defa tiklanilmasi icin bu kodu yazdim.
